    ...
    private void OnGUI()
    {
        ...
        if (GUILayout.Button("创建"))
        {
            bool isConfirmed = EditorUtility.DisplayDialog(
                "创建分支", // 弹窗标题
                "你确定要从" + original_branchName + "创建"+ new_branchName + "分支吗？", // 弹窗消息
                "Yes", // 确认按钮文本
                "No" // 取消按钮文本
            );
        ...
        if (GUILayout.Button("删除"))
        {
            bool isConfirmed = EditorUtility.DisplayDialog(
                "删除分支", // 弹窗标题
                "你确定要将" + delete_branchName + "分支删除吗", // 弹窗消息
                "Yes", // 确认按钮文本
                "No" // 取消按钮文本
            );
            if (!isConfirmed) return;
        ...
        if (GUILayout.Button("合并"))
        {
                ...
                !string.IsNullOrEmpty(merged_branchName) && !string.IsNullOrEmpty(merge_body))
            {
                bool isConfirmed = EditorUtility.DisplayDialog(
                    "合并分支", // 弹窗标题
                    "你确定要将" + merge_branchName + "合并到" + merged_branchName + "分支吗？", // 弹窗消息
                    "Yes", // 确认按钮文本
                    "No" // 取消按钮文本
                );
                if (!isConfirmed) return;
        ...
        if (GUILayout.Button("导入"))
        {
            if (!string.IsNullOrEmpty(load_branchName))
            {
                bool isConfirmed = EditorUtility.DisplayDialog(
                    "导入分支", // 弹窗标题
                    "你确定要将" + load_branchName + "导入到" + load_path + "吗？", // 弹窗消息
                    "Yes", // 确认按钮文本
                    "No" // 取消按钮文本
                );
                if (!isConfirmed) return;
